...
import StructureContent from "../model/structureContentModel.js";
...
const getContents = asyncHandler(async (req, res) => {
    ...
    try {
        let content = await StructureContent.find()
        if (content.length === 0) {
...
const getContent = asyncHandler(async (req, res) => {
    ...
    try {
        var content = await StructureContent.findById({ _id: id })
...
const sortContent = asyncHandler(async (req, res) => {
    ...
    try {
        var content = await StructureContent.find().sort(-1)
...
const updateContent = asyncHandler(async (req, res) => {
    var content = await StructureContent.findById(req.body.id)
...
const updateWithImage = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const contents = await StructureContent.findById(req.body.id)
...
const deleteContent = asyncHandler(async (req, res) => {
    ...
    try {
        const content = await StructureContent.findById(id);
        ...
        const result = await StructureContent.findByIdAndDelete({ _id: id })
