...
const api = 'https://spiritan.vercel.app/api/formation'
const Api = 'https://spiritan.vercel.app/api/formation/upload'
let data
let pending = false
  ...
  async function updateDataWithImage(formData) {
    ...
    try {
      ...
      if (response.status === 400 || response.status === 404) {
        ...
        console.log(json?.message)
        // return data = json?.content
        window.location.href = "/spiritual/formation-index.html"
      }
      // pending = false
    } catch (error) {
